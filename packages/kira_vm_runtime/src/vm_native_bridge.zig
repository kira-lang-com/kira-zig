...
const closure_bridge = @import("vm_native_closure_bridge.zig");
...
// Native closure/callback bridge — implemented in vm_native_closure_bridge.zig,
// re-exported here so existing `native_bridge.*` call sites and the `Vm` facade
// wrappers are unchanged.
pub const materializeNativeClosure = closure_bridge.materializeNativeClosure;
pub const materializeCallbackValueFromNative = closure_bridge.materializeCallbackValueFromNative;
pub const exportRuntimeClosureToNative = closure_bridge.exportRuntimeClosureToNative;
...
pub fn copyValueToNativeLayout(self: *Vm, module: *const bytecode.Module, ty: bytecode.TypeRef, value: runtime_abi.Value) anyerror!runtime_abi.Value {
    return switch (ty.kind) {
        ...
        .raw_ptr => try closure_bridge.copyCallbackValueToNativeLayout(self, module, ty, value),
        else => value,
    };
}
