...
pub fn lowerValue(ctx: *Context, expr: *model.Expr) anyerror!mid.Value {
    return switch (expr.*) {
        ...
        .string_from_scalar => |node| .{ .string_from_scalar = .{
            .inner = try allocValue(ctx, try lowerValue(ctx, node.operand)),
            .ty = node.ty,
            .temp_id = nextTempId(ctx),
            .span = node.span,
        } },
        .string_char_at => |node| .{ .string_char_at = .{
            .string = try allocValue(ctx, try lowerValue(ctx, node.object)),
            .index = try allocValue(ctx, try lowerValue(ctx, node.index)),
            .ty = node.ty,
            .temp_id = nextTempId(ctx),
            .span = node.span,
        } },
        .string_substring => |node| .{ .string_substring = .{
            .string = try allocValue(ctx, try lowerValue(ctx, node.object)),
            .start = try allocValue(ctx, try lowerValue(ctx, node.start)),
            .end = try allocValue(ctx, try lowerValue(ctx, node.end)),
            .ty = node.ty,
            .temp_id = nextTempId(ctx),
            .span = node.span,
        } },
        .string_index_of => |node| .{ .string_index_of = .{
            .string = try allocValue(ctx, try lowerValue(ctx, node.object)),
            .needle = try allocValue(ctx, try lowerValue(ctx, node.needle)),
            .ty = node.ty,
            .temp_id = nextTempId(ctx),
            .span = node.span,
        } },
